...
export function Onboarding({ onFinish, onSkip }: {
  ...
  const [debt, setDebt] = useState({ name: '', balance: '' as number | '', monthly: '' as number | '' });
        ...
        <>
          ...
          <div style={{ flex: '1 1 auto', overflowY: 'auto', padding: '8px 18px 16px' }}>
            {meta.key === 'income' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                <div>
                  ...
                  <Segmented value={draft.salary.cadence} options={[{ value: 'weekly', label: 'Weekly' }, { value: 'fourweek', label: '4-weekly' }, { value: 'monthly', label: 'Monthly' }]} onChange={(v) => setSalary('cadence', v)} />
                </div>
                <div>
            ...
            {meta.key === 'expenses' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {draft.expenses.map((e) => (
                  <Row key={e.id} catKey={e.category} title={e.name} meta={(CATEGORIES[e.category] || CATEGORIES.other).label} amount={fmtEur(Number(e.amount) || 0)} onRemove={() => removeFrom('expenses', e.id)} />
                ))}
            ...
            {meta.key === 'debts' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {draft.debts.map((d2) => (
                  <Row key={d2.id} icon="card" color="var(--violet)" title={d2.name} meta={fmtEur(Number(d2.monthly) || 0) + '/mo'} amount={fmtEur(Number(d2.balance) || 0)} onRemove={() => removeFrom('debts', d2.id)} />
                ))}
                <QuickAdd canAdd={!!debt.name && debt.balance !== ''} onAdd={() => { if (!debt.name || debt.balance === '') return; pushTo('debts', { ...debt, id: uid('d'), start: Number(debt.balance) || 0, apr: '' } as Debt); setDebt({ name: '', balance: '', monthly: '' }); }}>
