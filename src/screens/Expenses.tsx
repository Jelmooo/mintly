...
type Sort = 'payday' | 'amount' | 'category';

export function Expenses({ state, b, update }: { state: AppState; b: Budget; update: Upd }) {
  const [edit, setEdit] = useState<Expense | null>(null);
  const [sort, setSort] = useState<Sort>('payday');

  function save(item: Expense) {
    if (item.id) update((s) => ({ ...s, expenses: s.expenses.map((e) => (e.id === item.id ? item : e)) }));
    else update((s) => ({ ...s, expenses: [...s.expenses, { ...item, id: uid('e') }] }));
    setEdit(null);
  }
  const del = (id: string) => update((s) => ({ ...s, expenses: s.expenses.filter((e) => e.id !== id) }));
  const addCat = (cat: CustomCat) => update((s) => ({ ...s, customCats: [...(s.customCats || []), cat] }));
  const cats = b.cats;

  const list = [...state.expenses].sort((a, c) => {
    if (sort === 'amount') return Number(c.amount) - Number(a.amount);
      ...
      <Card>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, gap: 10, flexWrap: 'wrap' }}>
          ...
          <Segmented size="sm" value={sort}
            options={[{ value: 'payday', label: 'Payday' }, { value: 'amount', label: 'Amount' }, { value: 'category', label: 'Category' }]}
            onChange={setSort} />
        </div>
...
function PaydayTimeline({ expenses, cats }: { expenses: Expense[]; cats: Budget['cats'] }) {
  const byDay: Record<number, Expense[]> = {};
  expenses.forEach((e) => { (byDay[e.payday] = byDay[e.payday] || []).push(e); });
  const days = Object.keys(byDay).map(Number).sort((a, b) => a - b);
  const maxDay = 31;
  return (
    <div>
      <div style={{ position: 'relative', height: 2, background: 'var(--line)', borderRadius: 2, margin: '34px 6px 0' }}>
        {days.map((d) => {
          const total = byDay[d].reduce((s, e) => s + (Number(e.amount) || 0), 0);
            ...
            <div key={d} style={{ position: 'absolute', left: left + '%', top: 0, transform: 'translateX(-50%)' }}>
              <div style={{ position: 'absolute', bottom: 8, left: '50%', transform: 'translateX(-50%)', whiteSpace: 'nowrap', textAlign: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: 3, justifyContent: 'center', marginTop: 4 }}>
                  {byDay[d].map((e) => <span key={e.id} title={e.name} style={{ width: 7, height: 7, borderRadius: 2, background: (cats[e.category] || cats.other).color }} />)}
                </div>
              </div>
...
function ExpenseSheet({ item, cats, onAddCat, onClose, onSave }: {
  item: Expense | null; cats: Budget['cats']; onAddCat: (c: CustomCat) => void; onClose: () => void; onSave: (e: Expense) => void;
}) {
  const [d, setD] = useState<Expense | null>(item);
  const [mk, setMk] = useState<{ label: string; color: string; icon: string } | null>(null);
  useEffect(() => { setD(item); setMk(null); }, [item]);
  if (!d) return null;
  const set = (k: keyof Expense, v: unknown) => setD({ ...d, [k]: v } as Expense);

  function createCat() {
    if (!mk || !mk.label.trim()) return;
    const id = uid('c');
      ...
      <Field label="Category">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
          ...
          <button onClick={() => setMk(mk ? null : { label: '', color: CAT_COLORS[0], icon: 'cart' })} style={{
            ...
            border: `1px dashed ${mk ? 'var(--accent)' : 'var(--line)'}`, color: mk ? 'var(--accent)' : 'var(--text-3)',
          }}>
      ...
      {mk && (
        <div style={{ background: 'var(--bg-2)', border: '1px solid var(--line)', borderRadius: 'var(--r)', padding: 16, marginTop: -4, marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
            {CAT_COLORS.map((col) => (
              <button key={col} onClick={() => setMk({ ...mk, color: col })} title="color" style={{
                ...
                border: mk.color === col ? '2px solid var(--text)' : '2px solid transparent',
              }} />
            ))}
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
            ...
            <Btn variant="primary" size="sm" icon="check" onClick={createCat} disabled={!mk.label.trim()}>Create</Btn>
          </div>
        </div>
      )}
