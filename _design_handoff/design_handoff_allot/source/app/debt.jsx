// debt.jsx — track balance paid down + set monthly payment.
const { useState: useStateDebt } = React;

function Debt({ state, b, update }) {
  const [edit, setEdit] = useStateDebt(null);
  ...
  function del(id) { update((s) => ({ ...s, debts: s.debts.filter((x) => x.id !== id) })); }
  ...
  return (
    <div className="fadeup" style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <PageHead title="Debt" sub="Pay down balances with steady monthly payments from your main account."
        action={<Btn variant="primary" icon="plus" onClick={() => setEdit({ name: "", balance: "", monthly: "", apr: "" })}>Add debt</Btn>} />

      <StatRow items={[
        { label: "Total owed", value: fmtEur(totalBal), color: "var(--violet)" },
        { label: "Paid off so far", value: Math.round(paidPct) + "%", sub: fmtEur(totalStart - totalBal) + " of " + fmtEur(totalStart) },
        { label: "Monthly payments", value: fmtEur(b.debtTotal) },
      ]} />
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {state.debts.map((d) => {
          ...
          const months = d.monthly > 0 ? Math.ceil(bal / d.monthly) : Infinity;
          const payoff = isFinite(months) ? new Date(NOW.getFullYear(), NOW.getMonth() + months, NOW.getDate()) : null;
          return (
            <Card key={d.id}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18, gap: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 13 }}>
                  <div style={{ width: 40, height: 40, borderRadius: 11, background: "color-mix(in oklch, var(--violet) 16%, transparent)", color: "var(--violet)", display: "grid", placeItems: "center" }}><Icon name="card" size={21} /></div>
                  <div>
                    <h3 style={{ fontSize: 17 }}>{d.name}</h3>
                    <div style={{ display: "flex", gap: 8, marginTop: 5 }}>
                      {d.apr ? <Pill color="var(--amber)">{d.apr}% APR</Pill> : null}
                      {payoff && <Pill color="var(--text-2)"><Icon name="cal" size={11} /> clear {payoff.toLocaleDateString("en-GB", { month: "short", year: "numeric" })}</Pill>}
                    </div>
                  </div>
                </div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 9 }}>
                <div>
                  ...
                  <div className="num" style={{ fontSize: 28, fontWeight: 600, letterSpacing: "-0.03em" }}>{fmtEur(bal)}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div className="eyebrow">Paying</div>
                  <div className="num mono" style={{ fontSize: 17, color: "var(--violet)" }}>{fmtEur(d.monthly || 0)}/mo</div>
                </div>
              </div>
...
function DebtSheet({ item, onClose, onSave }) {
  const [d, setD] = useStateDebt(item);
  ...
  return (
    <Sheet open={!!item} onClose={onClose} title={isNew ? "Add debt" : "Edit debt"}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        {!isNew && <Field label="Original balance" hint="Used to show paid-down progress"><MoneyInput value={d.start} onChange={(v) => set("start", v)} /></Field>}
