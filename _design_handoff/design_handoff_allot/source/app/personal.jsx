// personal.jsx — the personal account: budgets + sweep-to-top-goal.
const { useState: useStatePers } = React;

function Personal({ state, b, update, onNav }) {
  const [edit, setEdit] = useStatePers(null);
  ...
  function del(id) { update((s) => ({ ...s, personalBudgets: (s.personalBudgets || []).filter((p) => p.id !== id) })); }
  ...
  return (
    <div className="fadeup" style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <PageHead title="Personal account" sub="Budget the money you pay yourself. Whatever's left sweeps into your top open goal."
        action={<Btn variant="primary" icon="plus" onClick={() => setEdit({ name: "", category: "groceries", amount: "" })}>Add budget</Btn>} />

      <StatRow items={[
        { label: "Paid to you / month", value: fmtEur(b.paidToPersonal), color: "var(--accent)" },
        { label: "Budgeted to spend", value: fmtEur(b.personalBudgetsTotal) },
        { label: top ? "Swept into " + top.name : "Unbudgeted", value: fmtEur(top ? b.sweep : Math.max(0, b.personalFree)), color: b.personalOver ? "var(--rose)" : "oklch(0.78 0.13 200)" },
      ]} />
      ...
      {b.personalOver && (
        <div style={{ display: "flex", gap: 12, alignItems: "center", padding: "14px 18px", background: "color-mix(in oklch, var(--rose) 12%, var(--surface))", border: "1px solid color-mix(in oklch, var(--rose) 35%, transparent)", borderRadius: "var(--r)" }}>
          ...
          <div style={{ fontSize: 13.5, color: "var(--text)" }}>Your personal budgets total <b>{fmtEur(b.personalBudgetsTotal)}</b>, which is <b>{fmtEur(-b.personalFree)}</b> more than the <b>{fmtEur(b.paidToPersonal)}</b> you pay yourself. Trim a budget or free up money upstream.</div>
        </div>
      )}

      <div className="pers-grid" style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 18, alignItems: "start" }}>
        {/* budgets list */}
        <Card>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 1, background: "var(--line-2)", borderRadius: "var(--r)", overflow: "hidden", border: "1px solid var(--line)" }}>
            ...
            {budgets.map((p) => {
              ...
              const share = b.paidToPersonal > 0 ? (p.amount / b.paidToPersonal) * 100 : 0;
              return (
                <div key={p.id} style={{ background: "var(--surface)", display: "flex", alignItems: "center", gap: 14, padding: "13px 16px" }}>
                  <CatDot category={p.category} cats={PCATS} size={36} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14.5, fontWeight: 500 }}>{p.name || cat.label}</div>
                    <div style={{ fontSize: 12, color: "var(--text-3)" }}>{cat.label} · {Math.round(share)}% of your pay</div>
                  </div>
                  <div className="num mono" style={{ fontSize: 15, width: 78, textAlign: "right" }}>{fmtEur(p.amount || 0)}</div>
            ...
            {top ? (
              <>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 12 }}>
                  <div style={{ width: 40, height: 40, borderRadius: 11, background: "color-mix(in oklch, oklch(0.78 0.13 200) 18%, transparent)", color: "oklch(0.78 0.13 200)", display: "grid", placeItems: "center" }}><Icon name="target" size={21} /></div>
                  <div>
                    <div style={{ fontSize: 16, fontWeight: 600 }}>{top.name}</div>
                    <Pill color="var(--text-2)" style={{ marginTop: 4 }}>no deadline · ongoing</Pill>
                  </div>
                </div>
                <div className="num" style={{ fontSize: 34, fontWeight: 600, letterSpacing: "-0.03em", color: "oklch(0.78 0.13 200)", marginTop: 16 }}>{fmtEur(b.sweep)}<span style={{ fontSize: 15, color: "var(--text-3)" }}> /mo</span></div>
                ...
                <button onClick={() => onNav("goals")} style={{ marginTop: 14, background: "var(--surface-2)", border: "1px solid var(--line)", color: "var(--text)", borderRadius: 10, padding: "9px 14px", fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}>Manage goals <Icon name="arrow" size={14} /></button>
              </>
            ) : (
              <div style={{ marginTop: 12 }}>
                ...
                <button onClick={() => onNav("goals")} style={{ marginTop: 14, background: "var(--surface-2)", border: "1px solid var(--line)", color: "var(--text)", borderRadius: 10, padding: "9px 14px", fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}>Add a goal <Icon name="arrow" size={14} /></button>
              </div>
            )}
          </Card>
...
function SplitBar({ budget, sweep, free }) {
  const total = Math.max(budget + sweep + free, 1);
  const segs = [
    { label: "Budgets", v: budget, color: "var(--accent)" },
    { label: "Saved", v: sweep, color: "oklch(0.78 0.13 200)" },
...
function BudgetSheet({ item, onClose, onSave }) {
  const [d, setD] = useStatePers(item);
